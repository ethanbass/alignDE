peakDetectionCWT <- function(ms, scales=c(1, seq(2,30,2), seq(32, 64, 4)),
                             SNR.Th = 3, peakThr = NULL, nearbyPeak = TRUE,
                             peakScaleRange = 5, amp.Th = 0.05,
                             minNoiseLevel = amp.Th/SNR.Th, ridgeLength = 24,
                             tuneIn = FALSE, ...) {
	...
	## The size of slide window changes over different levels,
	## with the coarse level have bigger window size
	if (is.null(amp.Th)) {
		amp.Th <- 0
	} else {
...
# the skip paramter should understand
	...
	majorPeakInfo <- identifyMajorPeaks(ms, ridgeList, wCoefs, SNR.Th = SNR.Th,
	                                    peakScaleRange = peakScaleRange, 
	...
	if (tuneIn) {
		...
		return(list(majorPeakInfo = refinedPeakInfo, ridgeList = ridgeList,
		            localMax = localMax, wCoefs = wCoefs[, -1], 
		            oldPeakInfo = majorPeakInfo))
	} else {
		return(list(majorPeakInfo = majorPeakInfo, ridgeList = ridgeList, 
		            localMax = localMax, wCoefs=wCoefs[, -1]))
	}
}
